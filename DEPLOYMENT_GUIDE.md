...
## 🔄 Proceso de Deployments Continuos

### Cómo Funciona el Auto-Deploy en Render

Render está configurado para hacer **auto-deploy** automáticamente cuando haces push a la rama `develop`. Esto significa que:

- ✅ Cada vez que haces `git push origin develop`, Render detecta el cambio
- ✅ Render automáticamente inicia un nuevo build y deploy
- ✅ No necesitas hacer nada manual en el dashboard (a menos que quieras)

### Flujo de Trabajo para Nuevos Deployments

#### Opción 1: Deploy Automático (Recomendado)

**Cuando hacer push a `develop` automáticamente despliega:**

1. **Desarrolla tu feature en una rama:**
   ```bash
...
2. **Mergea a develop:**
   ...
   # O usa un Pull Request en GitHub
   ```

3. **Push a develop (esto dispara el deploy automático):**
   ```bash
   git push origin develop
   ```
...
4. **Render automáticamente:**
   - Detecta el push
   - Inicia un nuevo build (puedes verlo en el dashboard)
   - Despliega la nueva versión
   - ⏳ Tarda aproximadamente 3-5 minutos

5. **Monitorea el deploy:**
   - Ve a tu servicio en Render Dashboard
   - Click en la pestaña **"Events"** o **"Logs"**
   - Verás el progreso del build y deploy en tiempo real

#### Opción 2: Deploy Manual

**Si quieres controlar cuándo se despliega:**

1. **Desactiva Auto-Deploy (opcional):**
   - En Render Dashboard > Tu Servicio > Settings
   - Desactiva **"Auto-Deploy"**
   - Ahora solo se despliega cuando lo inicies manualmente

2. **Para hacer deploy manual:**
   - En Render Dashboard > Tu Servicio
   - Click en **"Manual Deploy"**
   - Selecciona el commit o branch que quieres desplegar
   - Click **"Deploy"**
...
1. **En Render Dashboard:**
   - Ve a tu servicio
   - La pestaña **"Events"** muestra el historial de deployments
   - Verás estados: `Building` → `Deploying` → `Live` o `Failed`

2. **En los Logs:**
   - Click en **"Logs"** para ver el output en tiempo real
   - Verás el proceso de build, instalación de dependencias, y startup
...
```bash
# 1. Verifica que el servicio está activo
...
# ✅ "Servidor iniciando en el puerto..."
```

### Rollback (Revertir a Versión Anterior)

Si un deploy falla o introduces un bug:

#### Opción 1: Revertir el Commit en Git

```bash
# 1. Revertir el último commit en develop
git revert HEAD
git push origin develop
# Esto disparará un nuevo deploy automático con la versión anterior
```

#### Opción 2: Deploy Manual de un Commit Anterior

1. En Render Dashboard > Tu Servicio
2. Click **"Manual Deploy"**
3. Selecciona un commit anterior que sabes que funcionaba
4. Click **"Deploy"**

#### Opción 3: Usar el Historial de Render

1. En Render Dashboard > Tu Servicio > **"Events"**
2. Encuentra un deploy exitoso anterior
3. Click en los tres puntos (...) de ese deploy
4. Selecciona **"Redeploy"**

### Mejores Prácticas para Deployments

#### 1. **Usa Pull Requests para Code Review**

```bash
# En lugar de mergear directamente a develop:
# 1. Crea un PR desde tu feature branch a develop
# 2. Revisa el código
# 3. Mergea el PR
# 4. El auto-deploy se dispara automáticamente
```

#### 2. **Prueba Localmente Antes de Deployar**

```bash
# Siempre prueba localmente antes de push a develop
go run ./cmd/api
...
git commit -m "refactor: mejorar estructura de handlers"
...
#### 4. **Deploya en Horarios de Bajo Tráfico**

- Si es posible, deploya cuando hay menos usuarios activos
- Los deploys en Render pueden causar 10-30 segundos de downtime

#### 5. **Monitorea los Logs Después del Deploy**

- Revisa los logs durante los primeros 5-10 minutos después del deploy
- Busca errores o warnings
- Verifica que las conexiones a las bases de datos funcionan

### Configuración Avanzada de Deployments

#### Branch Protection (Opcional)

Si quieres más control, puedes:

1. **Usar diferentes servicios para diferentes branches:**
   - `develop` → Servicio de staging
   - `main` → Servicio de producción
   - Esto requiere crear dos servicios en Render

2. **Desactivar Auto-Deploy y usar deploys manuales:**
   - Útil si quieres controlar exactamente cuándo se despliega
   - Settings > Auto-Deploy > Desactivar

#### Notificaciones de Deploy

Render puede enviarte notificaciones cuando:
- Un deploy comienza
- Un deploy falla
- Un deploy se completa exitosamente

Configura esto en: Settings > Notifications

### Troubleshooting de Deployments

#### Problema: El deploy no se inicia automáticamente

**Soluciones:**
1. Verifica que Auto-Deploy esté activado en Settings
2. Verifica que estás haciendo push a la rama correcta (`develop`)
3. Verifica que el repositorio esté correctamente conectado

#### Problema: El build falla

**Revisa:**
1. Los logs del build en Render
2. Verifica que el Build Command sea correcto: `go build -tags netgo -ldflags '-s -w' -o app ./cmd/api`
3. Verifica que todas las dependencias estén en `go.mod`
4. Verifica que no haya errores de compilación localmente

#### Problema: El deploy se completa pero la app no funciona

**Revisa:**
1. Los logs de runtime (no los de build)
2. Verifica que las variables de entorno estén configuradas
3. Verifica que las conexiones a las bases de datos funcionen
